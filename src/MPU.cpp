...
#elif CONFIG_MPU_SPI
    constexpr auxi2c_config_t kAuxI2CConfig = {.clock = AUXI2C_CLOCK_400KHZ,
                                               .multi_master_en = 1,
                                               .sample_delay = 0,
                                               .shadow_delay_en = 0,
                                               .wait_for_es = 0,
...
// keep previous sensitivity value
    ...
    if (mode == MAG_MODE_POWER_DOWN) {
        ...
        const auxi2c_slv_config_t kSlaveReadDataConfig = {
                ...
                .reg_dis = 0,
                .sample_delay_en = 1,
                {{.swap_en = 0, .end_of_word = (auxi2c_eow_t) 0, .rxlength = 8}}};
        if (MPU_ERR_CHECK(setAuxI2CSlaveConfig(kSlaveReadDataConfig))) return err;

        // slave 1 changes mode to single measurement
        const auxi2c_slv_config_t kSlaveChgModeConfig = {.slave = MAG_SLAVE_CHG_MODE,
                .addr = COMPASS_I2CADDRESS,
                .rw = AUXI2C_WRITE,
